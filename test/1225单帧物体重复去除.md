...
## Stage 5：2D 候选生成机制 Ablation（YOLO box → FastSAM Prompt/Filter）
**假设 H6**：3D 重复（hit_ge2）与红线（best0_ge100）的根源在于 2D 分割的过分割（over-segmentation）与碎片化。通过引入 YOLOv8 检测框作为 **FastSAM 的 box prompt（官方 FastSAMPrompt.box_prompt 选择逻辑）**，期望输出更少、更“物体级”的候选，从源头减少碎片、降低重复，并尽量不牺牲覆盖率。

**关键澄清（因果口径必须统一）**：FastSAM 的 “box prompt” 本质仍是 **在候选 masks 上做选择/聚合**（官方选择器），并非像 SAM(SamPredictor) 那样根据 box 条件“生成新 mask”。因此本阶段的主实验要验证的是：**用官方 prompt 选择器（而非手写过滤器）在 YOLO box 约束下减少碎片，是否能降低 3D 重复且保红线。**

**实验设计**：
本轮只改变 **2D 数据预处理**（`load_scannet_mv_data_fast.py`），3D 端评测配置（test_cfg）完全固定。

### 实验矩阵
| Exp ID | 2D 策略 | 关键参数 | 预期目标 |
| :--- | :--- | :--- | :--- |
| **Exp0** | Baseline (FastSAM Everything) | 默认 | 建立 hit_ge2 / 红线基准 |
| **Exp1** | YOLOv8 → FastSAMPrompt.box_prompt（主实验） | conf=0.25, iou=0.65, expand=1.10, 1-mask/box | 验证“少而精”能否降重复且保红线 |
| **Exp2** | YOLOv8 → FastSAMPrompt.box_prompt（更保守召回） | conf=0.15, 其余同 Exp1 | 若 Exp1 红线变差，验证是否因漏检/覆盖下降导致 |
| **Exp0'** | Baseline + Top-M Cap | 仅保留 Top-M 个 masks (M=Exp1均值) | 对照组：区分收益来自“数量少”还是“边界好” |

> 备注（可选对照，先不必跑）：`yolo_mask_filter`（FastSAM everything → 用 YOLO box 做手写选择/过滤）只用于验证“官方 prompt vs 手写过滤器”的差异，避免把选择器不一致引入主结论。

### 必须记录的统计指标（用于归因）
每次实验需记录以下两组统计（通过脚本日志或额外 dump）：
1. **2D 侧（过分割程度）**：
  - `num_boxes`：YOLO 框数（用于排查漏检/阈值过严）。
  - `num_masks_raw`：FastSAM everything 的原始候选 mask 数。
  - `num_masks_used`：最终写入 group_ids 的 mask 数（越少通常越不碎）。
  - `pixel_coverage`：最终 masks 覆盖像素比例（防止“变少=漏检/覆盖下降”的假象）。
  - `mean_mask_area` / `median_mask_area`：mask 尺度变化（碎片化通常对应面积变小）。
2. **3D 侧（结构变化）**：
   - `num_superpoints_per_scan`：最终投影到 3D 的超点数量。
   - `hit_ge2`（重复率）：核心优化目标。
   - `best0_ge100` / `hit0@0.1_ge100`：红线指标（不能显著上升）。

### 判读规则
1. **Exp1 vs Exp0**：
   - 若 `hit_ge2` ↓ 且红线不变 → **成功**，YOLO+SAM 有效。
   - 若 `hit_ge2` ↓ 但红线 ↑ → **漏检/框太紧**，需调优 Exp1a 或 box expand。
   - 若 `hit_ge2` 不变 → **无效**，过分割未被抑制（检查 num_masks）。

2. **Exp2 vs Exp1**：
  - 若 Exp2 的红线更稳但 `hit_ge2` 回升 → Exp1 可能靠“更少”在工作，需结合 `pixel_coverage` 判断是否漏检。
  - 若 Exp2 与 Exp1 同向改善且覆盖不掉 → 说明 Exp1 的改善更可信（不是靠漏检）。

3. **Exp1 vs Exp0'**：
   - 若 Exp1 显著优于 Exp0' → 收益来自 **Prompt 带来的边界质量提升**。
   - 若 Exp1 ≈ Exp0' → 收益仅来自 **数量减少**，无需引入 YOLO，直接截断即可。
