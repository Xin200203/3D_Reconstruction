...
class LoadAdjacentDataFromFile(BaseTransform):
    ...
    def _load_rec_3d(self, results, pts_filenames):
        ...
        rec_pts_filename = os.path.join('data', self.dataset_type, 'points', f'{scene_name}.bin')
        rec_ins_path = os.path.join('data', self.dataset_type, 'instance_mask', f'{scene_name}.bin')
        rec_sem_path = os.path.join('data', self.dataset_type, 'semantic_mask', f'{scene_name}.bin')

        def _load_arr(filename: str, dtype):
            try:
                return np.frombuffer(get(filename, backend_args=self.backend_args), dtype=dtype)
            except ConnectionError:
        ...
        try:
            ...
            if '/points/' in frame_pts_filename:
                frame_ins_path = frame_pts_filename.replace('/points/', '/instance_mask/', 1)
                frame_sem_path = frame_pts_filename.replace('/points/', '/semantic_mask/', 1)
            else:
                parts = frame_pts_filename.split(os.sep)
                if 'points' in parts:
                    pidx = parts.index('points')
                    frame_ins_path = os.sep.join(parts[:pidx] + ['instance_mask'] + parts[pidx + 1:])
                    frame_sem_path = os.sep.join(parts[:pidx] + ['semantic_mask'] + parts[pidx + 1:])
                else:
                    raise
        ...
        if self.dataset_type == 'scannet' or self.dataset_type == 'scannet200':
            segment_path = os.path.join('data', self.dataset_type, 'scans', scene_name,
            ...
            segment_path = os.path.join('data', self.dataset_type, '3RScan', scene_name,
            ...
            segment_path = os.path.join('data', self.dataset_type, 'mesh_segs', f'{scene_name}.segs.json')
            try:
                segment_ids = np.array(json.load(open(segment_path))['segIndices'])
            except FileNotFoundError:
                segment_ids = np.array([])
