...
## 训练阶段配置建议（对应 `sv_bifusion_scannet200_3dmv_new.py`）

| 阶段 | 目标 | 建议配置修改 | 备注 |
| --- | --- | --- | --- |
| **Phase A** 3D-only 预热 | 打稳 3D baseline（无 2D 信号） | 使用原始配置 `sv_bifusion_scannet200_3dmv.py` 或 `two_d_losses` 全 False / 权重 0。 | `BiFusionEncoder` 可运行，但 2D 分支输入会被置零；decoder 维度保持 128。 |
| **Phase B** 对齐预训 | 冻结 3D 主干/decoder，小权重驱动 2D 对齐与像素重建 | 在 `sv_bifusion_scannet200_3dmv_new.py` 中启用：`two_d_losses.enable_recon=True`, `enable_seg=True`, 权重可设 `{w_recon:0.01,w_seg:0.01,w_align:0.02}`；warmup 推荐 `alpha_warmup=1000`, `recon/seg/align_warmup=1000`。 | 训练脚本可直接 `python tools/train.py ...`；Alpha 将从 0→1 渐升，2D-loss 逐步接入。 |
| **Phase C** 联合微调 | 全量解冻，保持小权重 2D 正则 | 同 Phase B 配置即可；若需要，可在中后期按验证表现调整 `{w_recon,w_seg,w_align}`。 | 观察 `cos_2d3d_mean_ln`、`norm_ratio`、`grad_ratio`，确保 2D/3D 学习均衡。 |
| **Phase D** 收尾抛光 | 去正则、专注 3D 指标 | 将 `w_recon/w_seg/w_align` 减小或设 0；`alpha_max` 可保持 1，但权重 0 时 2D 分支梯度自动消失。 | 若 2D 任务需保留，可保留极小权重（如 0.001）防止发散。 |

- 默认新配置 `sv_bifusion_scannet200_3dmv_new.py` 直接使用 Phase B/Phase C 的建议值，可立即运行 `tools/train.py`（确保 `conda activate ESAM` 与数据准备齐全）。
- 若希望沿用旧基线，只需切回 `sv_bifusion_scannet200_3dmv.py` 或将新配置中的 `two_d_losses` 改回默认（所有 enable=False、权重 0）。
